...
clean_tables <- function(df, section, subsection, geography){
	...
	if(section == "birth to prek" & subsection == "scholarships"){
		...
		r <- m %>%
			...
			dplyr::mutate_at(dplyr::vars(data1, data2, data3), ~purrr::map(.x, ~tidyr::separate(.x, count_frac, into = c("count", "frac"), sep = " "))) %>%
			dplyr::mutate_at(dplyr::vars(data1, data2, data3), ~purrr::map(.x, ~dplyr::mutate(.x, count = dplyr::case_when(count == "N/A" ~ 0, count == "CTSTR" ~ NA_real_, TRUE ~ readr::parse_number(count))))) %>%
			dplyr::mutate_at(dplyr::vars(data1, data2, data3), ~purrr::map(.x, ~dplyr::mutate(.x, frac = stringr::str_replace_all(frac, "\\(|%|\\)|>", ""))))
	...
	if(section == "birth to prek" & subsection == "early childhood screening"){
		...
		r <- m %>%
			...
			dplyr::mutate_at(dplyr::vars(data1, data2), ~map(.x, ~tidyr::separate(.x, count_frac, into = c("count", "frac"), sep = " "))) %>%
			dplyr::mutate_at(dplyr::vars(data1, data2), ~map(.x, ~dplyr::mutate(.x, count = dplyr::case_when(count == "N/A" ~ 0, count == "CTSTR" ~ NA_real_, TRUE ~ readr::parse_number(count))))) %>%
			dplyr::mutate_at(dplyr::vars(data1, data2), ~map(.x, ~dplyr::mutate(.x, frac = stringr::str_replace_all(frac, "\\(|%|\\)|>", ""))))
	...
	if(section == "kindergarten" & subsection == "child demographics"){
		...
		r <- m %>%
			...
			dplyr::mutate_at(dplyr::vars(data1, data2, data3), ~purrr::map(.x, ~tidyr::separate(.x, count_frac, into = c("count", "frac"), sep = " "))) %>%
			dplyr::mutate_at(dplyr::vars(data1, data2, data3), ~purrr::map(.x, ~mutate(.x, count = dplyr::case_when(count == "N/A" ~ 0, count == "CTSTR" ~ NA_real_, TRUE ~ readr::parse_number(count))))) %>%
	...
	if(section == "kindergarten" & subsection == "economic and food assistance"){
		...
		r <- m %>%
			...
			dplyr::mutate_at(dplyr::vars(data1, data2, data3), ~purrr::map(.x, ~tidyr::separate(.x, count_frac, into = c("count", "frac"), sep = " "))) %>%
			dplyr::mutate_at(dplyr::vars(data1, data2, data3), ~purrr::map(.x, ~dplyr::mutate(.x, count = case_when(count == "N/A" ~ 0, count == "CTSTR" ~ NA_real_, TRUE ~ readr::parse_number(count))))) %>%
			dplyr::mutate_at(dplyr::vars(data1, data2, data3), ~purrr::map(.x, ~dplyr::mutate(.x, frac = stringr::str_replace_all(frac, "\\(|%|\\)|>", "")))) %>%
